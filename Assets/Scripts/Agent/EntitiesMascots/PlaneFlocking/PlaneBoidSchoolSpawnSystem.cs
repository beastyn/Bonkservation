...
namespace Agent
{
    ...
    [BurstCompile]
    public partial struct PlaneBoidSchoolSpawnSystem : ISystem
    {
        private EntityQuery _boidQuery;
        public void OnCreate(ref SystemState state)
        {
            _boidQuery = new EntityQueryBuilder(Allocator.Temp).WithAll<PlaneBoid, LocalTransform>().Build(ref state);
        }
        ...
        public void OnUpdate(ref SystemState state)
        {
                         ...
                         .WithEntityAccess())
            {
                ...
                if (!state.EntityManager.Exists(boidSchool.ValueRO.Prefab))
                {
                    UnityEngine.Debug.LogError("Prefab entity does not exist in EntityManager!");
                    return;
                }


                state.EntityManager.Instantiate(boidSchool.ValueRO.Prefab, boidEntities);
                var setBoidLocalToWorldJob = new SetPlaneBoidLocalToWorld
                {
    ...
    [BurstCompile]
    struct SetPlaneBoidLocalToWorld : IJobParallelFor
    {
        ...
        public void Execute(int i)
        {
            ...
            var dirXZ = math.normalizesafe(random.NextFloat2() - new float2(0.5f));
            var dir = new float3(dirXZ.x, 0.0f, dirXZ.y);
