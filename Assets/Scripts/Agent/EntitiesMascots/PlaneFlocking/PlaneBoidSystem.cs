...
using static Agent.BoidAuthoring;
using UnityEditor;

namespace Agent
{
    public partial struct PlaneBoidSystem : ISystem
    {
        [BurstCompile]
        public void OnUpdate(ref SystemState state)
        {
            var boidQuery = SystemAPI.QueryBuilder().WithAll<PlaneBoid>().WithAllRW<LocalToWorld>().Build();
            ...
            state.EntityManager.GetAllUniqueSharedComponents(out NativeList<PlaneBoid> uniqueBoidTypes, world.UpdateAllocator.ToAllocator);
        ...
        struct MergeCells : IJobNativeParallelMultiHashMapMergedSharedKeyIndices
        {
            ...
            public void ExecuteNext(int cellIndex, int index)
            {
                ...
                //cellSeparation[cellIndex] = new float3(cellSeparation[cellIndex].x + cellSeparation[cellIndex].x, cellSeparation[cellIndex].y, cellSeparation[cellIndex].z + cellSeparation[cellIndex].z);
        ...
        partial struct SteerBoidJob : IJobEntity
        {
            ...
            [ReadOnly] public NativeArray<float3> ObstaclePositions;
            [ReadOnly] public NativeArray<float3> TargetPositions;
            public PlaneBoid CurrentBoidVariant;
