namespace UI.Utils
{
    ...
    {
        [SerializeField] bool needGradient;
        ...
        void Update()
        {
            ...
            for (int w = 0; w < wordIndexes.Count; w++)
            {
                ...
                Vector3 offset = Wobble(Time.time * this.speed + w);
                ...
                for (int i = 0; i < wordLengths[w]; i++)
                {
                    ...
                    {
                        colors[index] = rainbow.Evaluate(Mathf.Repeat(Time.deltaTime + vertices[index].x * 0.001f, 1f));
