...
namespace BrainDesigner.Scripts.Utils
{
    ...
    {
        internal struct SetData<T> where T : NamedWithDropdown
        {
            ...
            internal NamedWithDropdown selectedElement;
            internal SelectionType selectionType;

            internal SetData(VisualElement parentVisualElement, ListView listView, VisualTreeAsset templateElement, float elementHeight, Set<T> setToLoad, Dictionary<T, VisualElement> typeToElement, NamedWithDropdown selectedElement, SelectionType selectionType)
            {
        ...
        internal static SetData<T> CreateSetList<T>(SetData<T> setData, Action<IEnumerable<object>> LoadElement, VisualElement additionalInfoContainer = null) where T : NamedWithDropdown
        {
            ...
            void OnButtonRemoveElementClick<Tp>(SetData<Tp> setData, Action<IEnumerable<object>> LoadElement, VisualElement additionalInfoContainer, int i, T elementReference) where Tp : NamedWithDropdown
            {
                ...
                else
                {
                    ...
                    LoadElement?.Invoke(new List<NamedWithDropdown> { setData.setToLoad.list[nextElementIndex] });
            ...
            void BindElement(VisualElement e, int i)
            {
                ...
                if (elementReference is not NamedWithDropdown && elementReference is not T)
                {
        ...
        internal static void LoadSetDropdown<T>(ToolbarMenu menuSet, List<int> listIdsOrdered, Dictionary<int, T> sets, Action<int> loadAction) where T:NamedWithDropdown
        {
